...
public class QuartzJobComponent {
    ...
    public void addJob(QuartzJobModule quartzModel) {
        if (quartzModel.verify()) {
            try {
                ...
                // 定义定时器 按新的cronExpression表达式构建一个新的trigger
    ...
    public void modifyJobTime(String jobName, String cron, Date startDate, Date endDate) {
        TriggerKey triggerKey = TriggerKey.triggerKey(jobName, QuartzConstant.QZ_TRIGGER_GROUP_NAME);
    ...
    public void modifyJob(String oldJobName, String jobName, Class jobclass, String cron) {
         ...
         */
        //获取旧的触发器和定时任务
        TriggerKey triggerKey = TriggerKey.triggerKey(oldJobName, QuartzConstant.QZ_TRIGGER_GROUP_NAME);
        JobKey jobKey = JobKey.jobKey(oldJobName, QuartzConstant.QZ_JOB_GROUP_NAME);
        try {
    ...
    public void removeJob(String jobName) {
        TriggerKey triggerKey = TriggerKey.triggerKey(jobName, QuartzConstant.QZ_TRIGGER_GROUP_NAME);
        JobKey jobKey = JobKey.jobKey(jobName, QuartzConstant.QZ_JOB_GROUP_NAME);
        try {
            Trigger trigger = (Trigger) scheduler.getTrigger(triggerKey);
            if (trigger == null) {
                return;
